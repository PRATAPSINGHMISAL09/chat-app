...
export const sendMessage =  async (req,res)=>{
    try {
        ...
        if(!conversation){                             //if there is no converstion we create a convo
            conversation = await Conversation.create({
                participants:[senderId , receiverId]
            })
        }
        const newMessage = new Message({            //new message contents
            senderId,
            receiverId,
            message,
        })
        if(newMessage){
            conversation.messages.push(newMessage._id); // pushing new msg in the chat
        }
